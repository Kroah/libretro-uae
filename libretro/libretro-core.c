   ...
   for (i = 0; i < MAX_FLOPPY_DRIVES; i++)
   {
      if (gui_data.df[i][0])
      {
         if (!led_state[RETRO_LED_DRIVES])
            led_state[RETRO_LED_DRIVES] = gui_data.drive_motor[i];

         switch (i)
         {
            case 0:
               led_state[RETRO_LED_DRIVE0] = gui_data.drive_motor[i];
               break;
            case 1:
               led_state[RETRO_LED_DRIVE1] = gui_data.drive_motor[i];
               break;
            case 2:
               led_state[RETRO_LED_DRIVE2] = gui_data.drive_motor[i];
               break;
            case 3:
               led_state[RETRO_LED_DRIVE3] = gui_data.drive_motor[i];
               break;
         }
      }
   }
